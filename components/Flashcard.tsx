    ...
    <div className="max-w-xl mx-auto w-full perspective-1000">
      ...
      <div className="mb-4 flex justify-between items-center text-sm font-medium text-gray-500">
        ...
        <button 
          ...
          className={`text-purple-600 hover:text-purple-700 flex items-center gap-1 transition-colors ${isAiLoading ? 'opacity-50 cursor-wait' : ''}`}
        >
           {isAiLoading ? (
      ...
      <div className="relative h-[450px] w-full" style={{ perspective: '1000px' }}>
        ...
        >
          ...
          {/* FRONT OF CARD */}
          <div 
            className="absolute inset-0 w-full h-full bg-white rounded-2xl p-8 flex flex-col items-center justify-center backface-hidden border-t-4 border-indigo-500"
          ...
          >
            ...
            <p className="text-gray-400 mt-6 text-sm italic">Tap to flip</p>
            
            <button 
                onClick={() => setIsFlipped(true)}
                className="absolute bottom-0 w-full p-4 bg-gray-50 text-gray-600 font-semibold rounded-b-2xl hover:bg-gray-100 transition-colors"
            >
                Show Answer
            </button>
          </div>
          ...
          {/* BACK OF CARD */}
          <div 
            className="absolute inset-0 w-full h-full bg-indigo-50 rounded-2xl p-8 flex flex-col justify-between backface-hidden rotate-y-180 border-t-4 border-indigo-600"
            style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}
          >
             <div className="overflow-y-auto flex-1 pr-2">
                <div className="mb-6">
                    <h4 className="text-sm uppercase tracking-wider text-indigo-400 font-bold mb-2">Answer</h4>
                    <p className="text-xl font-bold text-indigo-900 leading-snug">{question.answer}</p>
                </div>
                
                <div className="space-y-4">
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-indigo-100">
                        <h5 className="text-xs font-bold text-gray-500 uppercase mb-1">Code Reference</h5>
                        <p className="text-sm font-mono text-gray-700">{question.citation}</p>
                        <p className="text-sm italic text-gray-600 mt-1">"{question.code_text}"</p>
                    </div>
                    
                    {question.mnemonic && (
                        <div className="bg-purple-100 p-3 rounded-lg border border-purple-200">
                             <p className="text-sm text-purple-800 font-medium">💡 {question.mnemonic}</p>
                        </div>
                    )}
                </div>
             </div>
